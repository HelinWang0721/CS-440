...
def hapax(train, tags, alpha):
    ...
    # return h
     # Calculate the number of hapax words
    hapax_count = sum(1 for w in wc.values() if w == 1)

    # Calculate the number of words that appear once
    one_count = sum(1 for w in wc.values() if w == 1)

    # Calculate the number of words that appear twice
    two_count = sum(1 for w in wc.values() if w == 2)

    # Calculate the discount factor
    d = (one_count / hapax_count) * (two_count / one_count)

    h = {t: (sum(twc[t].get(w, 0) for w in hapax) + alpha * d) / (len(hapax) + alpha) for t in list(tags)}
    return h
