...
#           Q1:How do you find the neighbors of a state?
#           A1: call state.get_neighbors()
#           Q2:How do you know if you’ve reached the goal?
#           A2: call state.is_goal()
#           Q3:How can you hash state objects into a dictionary? 
#           A3: call hash(state)
#           Q4:How can you push state objects onto a sorted heap?
#           A4: call heapq.heappush(heap, state)
...
class WordLadderState(AbstractState):
    ...
    def __lt__(self, other):    
        return (self.dist_from_start + self.compute_heuristic(), self.tiebreak_idx) < (other.dist_from_start + other.compute_heuristic(), other.tiebreak_idx)
...
# TODManhattanO(IV): implement this method (also need it for parts V and VI)
#  distance between two points (a=(a1,a2), b=(b1,b2))
def manhattan(a, b):
...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for dx, dy in directions:
            nx, ny = x + dx, y + dy
            if 0 <= nx < 3 and 0 <= ny < 3:  # within the grid
                new_state = [row.copy() for row in self.state]  # copy the state
    ...
    def compute_heuristic(self):
        ...
        for i in range(3):
            for j in range(3):
                if self.state[i][j] != 0:  # ignore the empty tile
                    tile = self.state[i][j]
                    goal_i, goal_j = divmod(tile, 3)  # compute the goal position of the current tile
                    total += manhattan((i, j), (goal_i, goal_j))  # add the Manhattan distance
        return total
